...
def _health_snapshot(timeout: float = 1.0) -> tuple[bool, dict | None]:
...
def _health_ok(timeout: float = 1.0) -> bool:
    return _health_snapshot(timeout)[0]


def _transport_watchdog_reason(
    ...
    *,
    ...
    if not isinstance(health_payload, dict):
        return None
    remote = health_payload.get("remote")
    if not isinstance(remote, dict) or remote.get("configured") is not True:
        return None
    status = str(remote.get("status") or "").strip().lower()
    if status in {"auth_degraded", "rate_limited", "disabled", "unconfigured", "credential_missing"}:
        return None
    activity = remote.get("activity")
    if not isinstance(activity, dict):
        return None
    raw_finished = str(activity.get("last_poll_finished_at") or "").strip()
    if not raw_finished:
        return None
    try:
        ...
        age = (current.astimezone(timezone.utc) - finished.astimezone(timezone.utc)).total_seconds()
    except Exception:
        return None
    if age > max(5.0, float(stale_seconds)):
        return f"transport poll stale: {age:.1f}s since last_poll_finished_at"
    return None


def _watchdog_restart_reason(health_ok: bool, health_payload: dict | None, health_failures: int, *, now: datetime | None = None, stale_seconds: float = 45.0) -> str | None:
    if not health_ok:
        if int(health_failures) >= 3:
            return f"local runtime health failed {int(health_failures)} consecutive checks"
        return None
    return _transport_watchdog_reason(health_payload, now=now, stale_seconds=stale_seconds)
...
def _ensure_runtime_dependencies(runtime: Path) -> None:
    lock = runtime / "requirements-release-lock.txt"
    wheelhouse = runtime / "_wheelhouse"

    if lock.is_file() and wheelhouse.is_dir() and any(wheelhouse.glob("*.whl")):
        offline = [
            str(PYTHON),
            "-m",
            "pip",
        ...
        result = subprocess.run(offline, cwd=str(runtime))
        if result.returncode == 0:
            return
...
def _runtime_validation_test_paths(runtime: Path) -> list[str]:
    tests = Path(runtime) / "tests"
    if not tests.exists():
        return []
    return [
        str(path)
        for path in sorted(tests.glob("test_*.py"))
        if not path.name.startswith("test_supervisor_")
    ]
...
def _run_validation(runtime: Path) -> None:
    ...
    test_paths = _runtime_validation_test_paths(runtime)
    if test_paths:
        result = subprocess.run(
            [str(PYTHON), "-m", "pytest", "-q", *test_paths],
            cwd=str(runtime),
...
def _dpapi_read(path: Path) -> str | None:
    ...
    try:
        ...
        while True:
            ...
            if now >= next_health_check:
                ...
                health_ok, health_payload = _health_snapshot(timeout=1.5)
                health_failures = 0 if health_ok else health_failures + 1
                watchdog_reason = _watchdog_restart_reason(health_ok, health_payload, health_failures)
                if watchdog_reason:
                    restart_times = [t for t in restart_times if now - t < 60]
                    restart_times.append(now)
                    if len(restart_times) > 3:
                        _status(state="failed", runtime_pid=None, last_error="Runtime watchdog restarted repeatedly; automatic restart stopped", last_watchdog_restart_reason=watchdog_reason)
                        return 2
                    _status(state="runtime_restarting", last_error="Watchdog: " + watchdog_reason, last_watchdog_restart_reason=watchdog_reason, last_watchdog_restart_at=time.time())
                    _stop_runtime(proc)
                    time.sleep(0.5)
                    proc = _start_runtime()
                    health_failures = 0
